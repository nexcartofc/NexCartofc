    ...
    <footer className="mt-auto bg-white border-t border-slate-200">
          ...
          <div className="space-y-8">
            ...
            <div className="flex gap-4">
              {[Facebook, Twitter, Instagram, Youtube].map((Icon, i) => (
                <a key={i} href="#" className="rounded-xl bg-slate-100 p-3 text-slate-600 transition-all hover:bg-blue-600 hover:text-white hover:-translate-y-1">
                  <Icon size={18} />
                </a>
              ))}
            </div>
            ...
            <div key={idx}>
              ...
              <ul className="space-y-4 text-sm font-bold text-slate-500">
                {section.links.map((link, lIdx) => (
                  <li key={lIdx}>
                    <Link to={link.to} className="hover:text-blue-600 transition-colors uppercase tracking-tight">
                      {link.name}
                    </Link>
                  </li>
                ))}
      ...
      <div className="bg-slate-900 text-white px-4 md:px-8 py-4 flex flex-col md:flex-row items-center justify-between text-[10px] font-bold uppercase tracking-[0.2em]">
        <div className="flex gap-8 mb-4 md:mb-0">
          <Link to="/seller" className="hover:text-blue-400 transition-colors">Become a Seller</Link>
          <Link to="/admin" className="hover:text-blue-400 transition-colors">Admin Dashboard</Link>
          <Link to="/track" className="hover:text-blue-400 transition-colors">Order Tracking</Link>
        </div>
